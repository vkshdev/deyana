...
def test_voice_settings_default_to_muted_local_female_voice(
    ...
    assert {voice["gender"] for voice in status.json()["availableTtsVoices"]} == {
        "female"
    }
...
def test_voice_transcribes_with_local_provider_without_storing_audio(
    tmp_path, monkeypatch
) -> None:
    monkeypatch.setattr(LocalVoiceService, "provider_status", lambda _self: "available")
    monkeypatch.setattr(
        "deyana_core.voice.run_windows_stt",
        lambda _language, _duration: CommandResult(
            0, "Open the launch checklist.\n", ""
        ),
    )
    ...
    with make_client(tmp_path) as client:
        client.patch(
            "/voice/settings",
            json={"enabled": True, "muted": False, "listenSeconds": 4},
        )
...
def test_voice_tts_uses_only_selected_local_female_voice(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda text, voice, rate, volume: (
            spoken.append((text, voice)) or CommandResult(0, "", "")
        ),
    )
