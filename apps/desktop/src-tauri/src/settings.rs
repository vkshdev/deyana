...
pub fn read_settings(app: &AppHandle) -> DesktopSettings {
    ...
    serde_json::from_str::<DesktopSettings>(&content).unwrap_or_default()
}

pub fn write_settings(app: &AppHandle, settings: &DesktopSettings) -> Result<(), String> {
...
fn settings_path(app: &AppHandle) -> Result<PathBuf, String> {
    let config_dir = app
        ...
        .app_config_dir()
        .map_err(|error| error.to_string())?;
    let current = config_dir.join("desktop-settings.json");
    let legacy = config_dir.join("phase1-settings.json");
